...
### Fixed
- **Gitee manual Token setup**: Entering a personal access token now exposes repository loading and passes that one-time value to the provider API, allowing Skills Hub to identify repository visibility before the first sync even when Gitee browser authorization is unavailable. Repository loading uses Gitee's supported `visibility=all` filter, accepts its real response shape where the HTTPS Git URL is returned in `html_url` while `clone_url` is null, and automatically matches typed repository URLs with or without the `.git` suffix.
- **GitHub OAuth proxy reuse**: GitHub device authorization now uses the configured app proxy for device-code requests, token polling, and authorized-account validation. GitLab and Gitee OAuth remain independent of the GitHub proxy setting.
