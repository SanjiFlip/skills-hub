...
### Fixed
- **Gitee manual Token setup**: Entering a personal access token now exposes repository loading and passes that one-time value to the provider API, allowing Skills Hub to identify repository visibility before the first sync even when Gitee browser authorization is unavailable.
