...
- **Local desktop OAuth configuration**: Debug runs and local `npm run tauri:build*` commands automatically read only `SKILLS_HUB_GITHUB_CLIENT_ID` from the repository-root `.env` when neither the build environment nor an explicit configuration file provides it. Packaged builds still stop before compilation when the public Client ID is missing or malformed; Client Secrets and user tokens are never imported.
