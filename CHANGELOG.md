...
- **Desktop OAuth packaging**: Require a GitHub public Client ID before packaged builds. Local build commands automatically read only this key from the root `.env` when no build environment or explicit env file is provided; missing or malformed configuration still stops the build instead of shipping a disabled authorization button.
