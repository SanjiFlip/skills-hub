...
### 优化
- **内置工具目录**：内置适配器数量更新为 48，并在中英文支持矩阵中补充 ZCode 路径规则。
...
### 新增
- **ZCode 工具适配**：新增通过 `~/.zcode/skills` 和 `.zcode/skills` 进行全局及项目级 Skill 同步的支持。Skills Hub 会通过 `~/.zcode` 检测 ZCode，并在工具管理和同步流程中展示 Z.ai 产品图标（修复 [#144](https://github.com/qufei1993/skills-hub/issues/144)）。
