...
### 新增
- **显式的单向设备同步**：设备同步新增「从仓库拉取到本地」与「推送本地到仓库」。拉取会把仓库中的全部技能复制到本地库；推送会把本地库写入仓库。两者**只新增与更新、都不删除**，因此方向始终由你决定，单向操作绝不会移除内容。自动合并仍是唯一会处理删除的路径。

### 修复
- **设备同步不再删除它读不到的内容**：中央技能库目录缺失的技能在导出本地库时被跳过，因而从不进入本地清单。合并逻辑把这读成"本机已删除"，于是**删掉仓库里的副本并推送**，随之而来的远端删除又清掉本地记录——库丢内容最终把仓库也清空了。现在这类技能会被明确告知合并逻辑，改为**从仓库恢复**，而不是删除任何内容。
...
- **空壳技能不再显示为正常**：中央技能库中的目录已丢失的受管理技能，现在会如实报告为独立的问题状态，而不再显示为可用——包括没有外部来源的技能，此前这类技能会跳过状态检查、内容已丢失却显示为正常。当某个名称已存在这样的空壳记录时，采纳目录会**修复该记录**（重新指向实际存在的目录），从而保留其标签、启用状态与同步目标，且不会产生重复技能。
- **删除内容已丢失的技能**：中央技能库中的目录已不存在的技能，此前每次删除都会失败并报 `Skill content is missing`，留下一个既无法使用也无法移除的记录。这种情况没有可保留的内容，因此现在改为直接移除该记录及其工具链接。若设备同步的其他设备仍持有该技能，下次同步会用完整内容把它恢复回来。

## [0.10.2] - 2026-09-17

### 新增
- **通知区域图标**：Skills Hub 现在会在任务栏通知区域显示托盘图标，菜单提供「显示主窗口」与「退出 Skills Hub」，左键单击图标即可恢复窗口。菜单文字跟随界面语言。

### 变更
- **点击关闭改为隐藏到托盘**：窗口关闭按钮不再退出应用，而是隐藏窗口，让计划任务更新与设备同步继续在后台运行；需要退出时请使用托盘菜单。若托盘图标创建失败，关闭仍会退出应用，确保不会出现无法找回窗口的情况。

### 修复
- **后台更新完全无窗口**：计划任务与手动触发的自动更新不再弹出窗口。后台运行此前会在 `setup` 之前创建配置中声明的窗口，并在该窗口之后同步执行整个更新，导致每次更新期间都会出现一个可见且无法操作的窗口；现在改为直接跳过创建该窗口，同时也不再为无窗口运行启动 WebView2 与前端页面。「立即更新」改为在当前应用进程内执行，不再要求操作系统计划任务拉起第二个应用实例；SQLite 连接也会等待并发写入完成，不再立即失败。
- **自动更新计划任务注册**：「立即更新」不再重新注册操作系统计划任务。此前该操作会无条件注册，导致即使已关闭自动更新，计划任务仍会被重新创建，更新继续在用户不知情的情况下运行并弹出窗口。现在只有自动更新开关本身会安装或移除该任务。
