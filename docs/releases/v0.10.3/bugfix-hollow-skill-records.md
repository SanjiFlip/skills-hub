...
**三、删除内容已丢失的技能不再失败。** `recycle_bin.rs` 的 `archive` 以 `anyhow::ensure!(source.is_dir(), "Skill content is missing: {:?}", source)` 开头，因此中央库目录不存在时删除**必然失败**——用户既无法使用这条记录，也无法移除它，每次尝试都只会看到同一条报错。这种情况下没有任何内容需要保留，因此现在改为：移除记录（外键级联清理标签、同步目标与来源检查）并清理工具链接，不写入回收站条目（`trash_path` 为空）。若设备同步的其他设备仍持有该技能，下次同步会用完整内容将其恢复。

## 最终行为

中央库目录缺失的技能会明确显示为异常并给出原因（中央技能库中的该技能目录不存在），不再默默显示为正常。通过发现横幅采纳同名目录时，对应的空壳记录会被修复而不是复制成第二份，标签与同步目标得以保留。这类技能也可以直接删除，删除后不会在回收站留下无法恢复的条目。
